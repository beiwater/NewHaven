    ...
    <header className="flex items-center bg-gradient-to-r from-amber-900 via-amber-800 to-amber-900 text-white border-b-2 border-amber-700 shrink-0" style={{ height: 48 }}>
      ...
      >
        ...
        <div className="leading-tight min-w-0">
          ...
          <div className="text-[9px] text-amber-300/80">{t('topbar.tagline')}</div>
        </div>
      </button>

      {/* Right icons */}
      <div className="flex items-center h-full shrink-0">
        <button className="w-10 h-full flex items-center justify-center hover:bg-amber-700/30 transition-colors" title={t('topbar.notifications')}>
        ...
        <button onClick={() => setActiveView('settings')} className="w-10 h-full flex items-center justify-center hover:bg-amber-700/30 transition-colors" title={t('topbar.settings')}>
          <svg className="w-4.5 h-4.5 text-amber-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">

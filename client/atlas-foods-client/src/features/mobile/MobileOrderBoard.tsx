  ...
  return (
    <div className="bg-white/60 rounded-xl border border-amber-300/50 p-3 min-w-[260px] shrink-0">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-xs font-bold text-amber-800 uppercase tracking-wider">{t('mobile.orderBoard')}</h3>
      </div>

      {orders.length === 0 && (
        <div className="text-[10px] text-amber-400 italic text-center py-2">{t('mobile.noOrders')}</div>
      )}
            ...
            <div key={order.id} className="flex items-center gap-2 p-2 bg-amber-50/70 rounded-lg border border-amber-200/30">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[10px] font-semibold text-amber-900 truncate">
                  {t('mobile.deliverOrder', { quantity: order.quantity })} {resourceName(order.resourceId)}
                </div>
                <div className="text-[9px] text-amber-600">
                  {t('mobile.reward')}: ${rewardCash.toLocaleString()}
                  {rewardXP > 0 && <span className="ml-1">{rewardXP} {t('mobile.xp')}</span>}
                </div>
              </div>
              {isCompleted ? (
                <button

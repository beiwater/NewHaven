    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-lg p-4 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-amber-700/70">Bonuses & Buffs</p>
            <h2 className="text-xl font-black text-amber-950">Inspect</h2>
          </div>
        </div>
        ...
        <section>
          ...
          {activePowerups.length === 0 ? (
            <div className="text-[10px] text-amber-400 italic py-2">No active power-ups</div>
          ) : (
                ...
                <div key={p.type} className="flex items-center gap-3 p-3 bg-green-50/80 rounded-xl border border-green-200/40">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[10px] text-green-600">
                      Ends: {new Date(p.endsAt).toLocaleTimeString()}
                    </div>
                  </div>
                  <span className="text-[10px] font-bold text-green-700 bg-green-200/50 px-2 py-0.5 rounded-full">Active</span>
                </div>
              ))}
            </div>
          )}
        ...
        <section>
          ...
          {completedResearch.length === 0 ? (
            <div className="text-[10px] text-amber-400 italic py-2">No completed research yet</div>
          ) : (
            <div className="space-y-1.5">
              {completedResearch.map((r) => (
                <div key={r.id} className="flex items-center gap-3 p-3 bg-purple-50/80 rounded-xl border border-purple-200/40">
                  ...
                  <div className="flex-1">
                    ...
                    {r.unlockPct != null && (
                      <div className="text-[10px] text-purple-600">+{r.unlockPct}% quality</div>
                    )}
                  </div>
                  <span className="text-[10px] font-bold text-purple-700 bg-purple-200/50 px-2 py-0.5 rounded-full">Done</span>
                </div>
              ))}
            </div>
          )}
        ...
        <section>
          ...
          {unlockedFeatures.length === 0 ? (
            <div className="text-[10px] text-amber-400 italic py-2">No features unlocked yet</div>
          ) : (
              ...
              {Object.entries(unlocks.features)
                ...
                .map(([k]) => (
                  <div key={k} className="text-[10px] text-amber-400 flex items-center gap-1">
                    ...
                    <span>— unlocks at Lv.{unlocks.featureLevels?.[k] ?? '?'}</span>
                  </div>
                ))}
            </div>
          )}

...
export function TopBar() {
  ...
  const powerupStatus = activePowerupCount > 0 ? t('topbar.powerupActive', { count: activePowerupCount }) : t('topbar.powerupReady', { count: remainingPowerups })
  ...
  return (
    <header className="topbar flex items-center bg-gradient-to-r from-amber-900 via-amber-800 to-amber-900 text-white border-b-2 border-amber-700 shadow-md z-50">
      ...
      <button
        ...
        title={financeUnlocked ? t('topbar.viewFinancials') : t('topbar.financeLocked', { level: financeUnlockLevel })}
      >
        ...
        <div>
          ...
          {!financeUnlocked && <div className="text-[9px] text-amber-200/80">{t('nav.locked', { level: financeUnlockLevel })}</div>}
        </div>
      </button>
      ...
      <div className="flex items-center gap-3 px-5 h-full flex-1">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between text-xs">
            ...
            <span className="text-amber-300/80">{t('topbar.xpProgress', { current: currentXp.toLocaleString(), total: xpToNext.toLocaleString() })}</span>
          </div>
          <div className="mt-1 h-2.5 bg-amber-950/50 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-green-500 to-green-400 rounded-full transition-all duration-500" style={{ width: `${xpPct}%` }} />
          </div>
        </div>
      ...
      <div className="flex items-center gap-1 px-3 ml-auto">
        ...
        <a
          href="https://github.com/beiwater/NewHaven"
          target="_blank"

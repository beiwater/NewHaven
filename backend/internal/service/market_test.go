...
func TestCreateQualitySellOrderUsesQualityInventory(t *testing.T) {
	...
	s.State.Companies[0].QualityInventory = map[string]int{"3_2": 10}

	if _, err := s.CreateOrder(s.State.Companies[0].ID, 3, 0, 2, 4, 10); err != nil {
		t.Fatalf("CreateOrder() unexpected error: %v", err)
	}
	if got := s.State.Companies[0].QualityInventory["3_2"]; got != 6 {
		t.Fatalf("quality inventory = %d, want 6", got)
	}
	if got := s.State.Companies[0].Inventory[3]; got != 0 {
...
func TestCancelQualitySellOrderRefundsQualityInventory(t *testing.T) {
	...
	resp, err := s.CreateOrder(s.State.Companies[0].ID, 3, 0, 2, 4, 10)
	if err != nil {
		t.Fatalf("CreateOrder() unexpected error: %v", err)
	}
	...
	if got := s.State.Companies[0].QualityInventory["3_2"]; got != 10 {
...
func TestTakeQualitySellOrderAddsQualityInventory(t *testing.T) {
	...
	s.State.Orders = []model.MarketOrder{{
		ID: "sell-quality", ResourceID: 3, Kind: 0, Quality: 2,
		Quantity: 5, Remaining: 5, Price: 10, CompanyID: s.Cfg.Game.Bot1ID,
	}}

	if _, err := s.TakeOrder(s.State.Companies[0].ID, 3, 3, 2, 10); err != nil {
		t.Fatalf("TakeOrder() unexpected error: %v", err)
	}
	if got := s.State.Companies[0].QualityInventory["3_2"]; got != 3 {
...
func TestCreateOrderRejectsInvalidKind(t *testing.T) {
	s := newTestService()
	if _, err := s.CreateOrder(s.State.Companies[0].ID, 3, 7, 0, 1, 10); err == nil {
...
func TestCreateBuyOrderDeductsCash(t *testing.T) {
	...
	resp, err := s.CreateOrder(s.State.Companies[0].ID, 3, 1, 0, 5, 10.0)
	if err != nil {
...
func TestCreateSellOrderDeductsInventory(t *testing.T) {
	s := newCoreTestService()
	initial := s.State.Companies[0].Inventory[3]

	_, err := s.CreateOrder(s.State.Companies[0].ID, 3, 0, 0, 10, 12.0)
	if err != nil {
		t.Fatalf("CreateOrder sell: %v", err)
	}
	if got := s.State.Companies[0].Inventory[3]; got != initial-10 {
...
func TestCreateOrderInsufficientInventory(t *testing.T) {
	s := newCoreTestService()
	s.State.Companies[0].Inventory[3] = 5
	_, err := s.CreateOrder(s.State.Companies[0].ID, 3, 0, 0, 10, 12.0)
	if err == nil {
...
func TestCreateOrderInsufficientCash(t *testing.T) {
	...
	_, err := s.CreateOrder(s.State.Companies[0].ID, 3, 1, 0, 100, 100.0)
	if err == nil {
...
func TestCancelSellOrderRefundsInventory(t *testing.T) {
	...
	resp, err := s.CreateOrder(s.State.Companies[0].ID, 3, 0, 0, 10, 12.0)
	if err != nil {
		t.Fatalf("CreateOrder: %v", err)
	}
	order := resp["order"].(model.MarketOrder)
	...
	if got := s.State.Companies[0].Inventory[3]; got != initial {
...
func TestMatchLimitOrders(t *testing.T) {
	...
	s.State.Orders = []model.MarketOrder{
		{ID: "sell-1", ResourceID: 3, Kind: 0, Price: 10.0, Quantity: 20, Remaining: 20, CompanyID: s.Cfg.Game.Bot1ID, Status: "open"},
	}
	initialInv := s.State.Companies[0].Inventory[3]

	_, err := s.CreateOrder(s.State.Companies[0].ID, 3, 1, 0, 10, 10.0)
	if err != nil {
		t.Fatalf("CreateOrder buy: %v", err)
	}
	// After match, company should have inventory + filled quantity
	if got := s.State.Companies[0].Inventory[3]; got != initialInv+10 {
...
func TestMatchLimitOrdersPricePriority(t *testing.T) {
	...
	s.State.Orders = []model.MarketOrder{
		{ID: "sell-high", ResourceID: 3, Kind: 0, Price: 12.0, Quantity: 10, Remaining: 10, CompanyID: s.Cfg.Game.Bot1ID, Status: "open"},
		{ID: "sell-low", ResourceID: 3, Kind: 0, Price: 9.0, Quantity: 10, Remaining: 10, CompanyID: s.Cfg.Game.Bot2ID, Status: "open"},
	}

	_, err := s.CreateOrder(s.State.Companies[0].ID, 3, 1, 0, 10, 15.0)
	if err != nil {
		t.Fatalf("CreateOrder: %v", err)
	}

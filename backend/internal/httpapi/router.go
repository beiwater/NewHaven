...
func NewRouter(cfg *config.Config, h *RouterHandlers) *chi.Mux {
	...
	r.Route("/api/v2", func(r chi.Router) {
		...
		r.With(AuthRequired(cfg.JWTSigningKey)).Post("/players/{playerId}/preferences/", h.Player.handleSavePreferences)
		r.With(AuthRequired(cfg.JWTSigningKey)).Patch("/companies/me/story-progress/", h.Company.handleUpdateStoryProgress)
		r.With(AuthRequired(cfg.JWTSigningKey)).Post("/companies/me/tutorial/", h.Company.handleCompleteTutorial)
		...
		r.With(AuthRequired(cfg.JWTSigningKey)).Post("/buildings/{buildingId}/stock/", h.Building.handleStockShelf)
		r.With(AuthRequired(cfg.JWTSigningKey)).Post("/buildings/{buildingId}/unstock/", h.Building.handleUnstockShelf)
		r.With(AuthRequired(cfg.JWTSigningKey)).Post("/buildings/{buildingId}/shelf-price/", h.Building.handleSetShelfPrice)
	...
	r.Route("/api/v3", func(r chi.Router) {
		...
		r.With(AuthRequired(cfg.JWTSigningKey)).Get("/companies/{companyId}/buildings/", h.Building.handleListCompanyBuildings)
		...
		r.With(AuthRequired(cfg.JWTSigningKey)).Get("/companies/me/orders/", h.Market.handleListMyOrders)
	})

import { useProductionQueue, useProductionJobs, useCancelJob, useClaimProduction, useClaimAll } from '@/api/hooks/production.hooks'
...
export function ProductionQueue() {
  ...
  return (
    <div className="p-4">
      <h2 className="text-lg font-bold text-amber-900 mb-3">Production</h2>
      ...
      {queue && (
        <div className="flex gap-3 mb-4">
          <div className="flex-1 bg-white/60 rounded-lg p-3 border border-amber-200/40 text-center">
            ...
            <div className="text-[10px] text-amber-600 uppercase">Slots Used</div>
          </div>
          <div className="flex-1 bg-white/60 rounded-lg p-3 border border-amber-200/40 text-center">
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-xs font-semibold text-green-700 uppercase tracking-wider">
              Ready to Collect ({collectableJobs.length})
            </h3>
            <Button size="sm" variant="default" onClick={() => claimAll.mutate()}>
              Collect All
            </Button>
          </div>
          <div className="space-y-1.5">
            {collectableJobs.map((job) => (
              <div key={job.id} className="flex items-center gap-2 p-2 bg-green-50 rounded-lg border border-green-200 text-xs">
                ...
                <button
                  ...
                  className="ml-auto text-green-600 hover:text-green-800 font-semibold text-[10px]"
                >
                  Collect
                </button>
              </div>
      ...
      <div className="space-y-1.5">
        ...
        {activeJobs.map((job) => {
          ...
          return (
            <div key={job.id} className="flex items-center gap-2 p-2 bg-white/60 rounded-lg border border-amber-200/40 text-xs">
              <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
              <span className="text-amber-900 font-medium">{job.id.slice(0, 6)}</span>

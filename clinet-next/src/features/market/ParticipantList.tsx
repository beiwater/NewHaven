  ...
  return (
    <div className="rounded-2xl border border-amber-300/60 bg-white/60 p-4 shadow-sm">
      ...
      <div className="space-y-2">
        {orders.length === 0 && (
          <div className="rounded-lg bg-amber-50 px-3 py-4 text-center text-xs text-amber-500">{emptyText}</div>
        )}
        {orders.slice(0, 10).map((order, index) => (
          <div key={`${order.id}-${order.companyId}-${index}`} className="grid grid-cols-[1fr_auto] gap-2 rounded-lg border border-amber-200/70 bg-white/70 px-3 py-2 text-xs">
            <div className="min-w-0">
              <div className="truncate font-black text-amber-950">{companyLabel(order.companyId, currentCompanyId)}</div>
              <div className="text-[10px] text-amber-600">Order {shortOrderId(order.id)}</div>
            </div>
            <div className="text-right">
              <div className="font-black tabular-nums text-amber-950">${order.price.toFixed(2)}</div>
              <div className="text-[10px] font-semibold text-amber-700">{order.remaining.toLocaleString()} / {order.quantity.toLocaleString()}</div>
            </div>
          </div>
        ))}
      </div>

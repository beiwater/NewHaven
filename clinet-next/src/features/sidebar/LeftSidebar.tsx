...
export function LeftSidebar() {
  ...
  const extCompany = companyData as unknown as
    | {
        authCompany?: { level?: number; money?: number }
        levelInfo?: { level: number }
        unlocks?: { features?: Record<string, boolean>; featureLevels?: Record<string, number> }
      }
    | undefined

  const level = companyData?.levelInfo?.level ?? extCompany?.authCompany?.level ?? 1
  const cash = companyData?.authCompany?.money ?? 0
  const unlocks = extCompany?.unlocks
      ...
      <div className="mt-auto p-3 mx-2 mb-3 bg-amber-900/10 rounded-lg border border-amber-700/20">
        ...
        <div className="flex items-center justify-center gap-1">
          <svg className="w-3 h-3 text-yellow-600" fill="currentColor" viewBox="0 0 20 20">
            <path
              fillRule="evenodd"
              d="M10 1.944A11.954 11.954 0 012.166 5C2.056 5.647 2 6.319 2 7c0 5.225 3.34 9.67 8 11.317C14.66 16.67 18 12.225 18 7c0-.682-.057-1.353-.166-2A11.954 11.954 0 0110 1.944zM11 14a1 1 0 11-2 0 1 1 0 012 0zm0-7a1 1 0 10-2 0v3a1 1 0 102 0V7z"
              clipRule="evenodd"
            />
          </svg>

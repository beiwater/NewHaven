  ...
  return (
    <div className={`rounded-xl border-2 p-4 shadow-sm transition-shadow hover:shadow-md ${cardBg}`}>
      ...
      <div className="mb-3 space-y-1 border-t border-amber-200/50 pt-2">
        ...
        <StatRow label="Management" value={`${executive.mgmtDiscount}%`} tone="up" />
      ...
      {mode === 'market' && (
        <button
          ...
          className={`w-full rounded-lg py-2 text-xs font-black uppercase tracking-wider transition-colors ${
            canAfford
              ? 'bg-amber-800 text-white hover:bg-amber-900 disabled:bg-amber-400'
              : 'bg-gray-300 text-gray-500 cursor-not-allowed'
          }`}
        >
          {isPending ? 'Recruiting...' : canAfford ? `Recruit $${formatMoney(executive.recruitCost)}` : 'Cannot afford'}
        </button>
      )}
      ...
      {mode === 'owned' && (
        <div className="flex gap-2">
          ...
          >
            {isTraining ? trainingLabel : 'Train'}
          </button>
        </div>
      )}
    </div>
